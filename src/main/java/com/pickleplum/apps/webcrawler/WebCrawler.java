 ...
 * WebCrawler class
 * 
 * Main class for WebCrawler application
 * 
			...
			try {
				...
				if (rawURL.getHost().contentEquals(hostname) && rawURL.getPort() == port) {
	        		...
	        		try {
	        			...
	        			printLink(sep, url, "Unable to connect to link, unsupported MIME type");
	        		}
    			...
    			printLink(sep, url, "Unable to connect to link, malformed URL");
    		}
		} else {
